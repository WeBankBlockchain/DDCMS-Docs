...
.. admonition:: 什么是 WeBankBlockchain-DDCMS
    ...
    为解决以上问题，我们希望通过DDCMS项目(Distributed Data Collaboration  Management Solution)的建设，在多方协同合作场景中，面向数据管理需求，提供一套基于区块链的、安全可信、友好易用的分布式数据管理开源方案，服务于个人数据携带、企业间数据共享。在数据共享过程中，各参与方围绕数据目录展开各种业务协作，使得各参与方能够快速、高效的进行数据管理、数据需求对接、传输和使用数据的同时，确保数据的安全、可信、可靠、可追溯、可监管、可审计。
